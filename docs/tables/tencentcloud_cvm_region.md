...
Tencent Cloud CVM regions are physically isolated geographic locations where cloud resources are hosted. Each region has a unique ID (e.g., ap-guangzhou), a description (e.g., South China (Guangzhou)), and an availability state indicating whether the region is currently available.
